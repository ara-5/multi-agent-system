"""The ORIGINAL (naive) approach to training simple_speaker_listener: a single
PPO MlpPolicy over the concatenated speaker+listener observation. Kept for the
record, not as the recommended path -- see joint_env.py for why this uses a
single joint policy rather than the freeze-one-side approach used for
simple_tag, and see bottleneck_policy.py / train.py for why this specific
architecture plateaus at 13.5% of max mutual information between target and
message no matter how long you train it or how much entropy bonus you add:
nothing here ever requires the message to carry the target information, since
the one shared network that produces the listener's movement logits also has
direct access to the speaker's raw observation (the target). More training or
entropy on top of that only ever produces a weak incidental correlation.
"""
import argparse
import os
