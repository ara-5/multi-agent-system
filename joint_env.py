...
class JointPolicyEnv(gym.Env):
    ...
    def __init__(self, pz_env_fn):
        super().__init__()
        self._env = pz_env_fn()
        self.agent_order = list(self._env.possible_agents)
        obs_dims = [self._env.observation_space(a).shape[0] for a in self.agent_order]
        ...
        self.action_space = spaces.MultiDiscrete(
            [self._env.action_space(a).n for a in self.agent_order]
        )
    ...
    def reset(self, *, seed=None, options=None):
        obs, _infos = self._env.reset(seed=seed, options=options)
        return self._flatten_obs(obs), {}

    def step(self, action):
        actions = {agent: int(a) for agent, a in zip(self.agent_order, action)}
        obs, rewards, terminations, truncations, _infos = self._env.step(actions)
